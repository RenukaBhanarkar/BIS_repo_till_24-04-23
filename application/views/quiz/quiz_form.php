 ...
 <script type="text/javascript">
    $('#quiz_reg').submit(function(e) { 
                    ...
                    if (terms_conditions == "" || terms_conditions== null) {
                        if ($("#terms_conditions").next(".validation").length == 0) // only add if not added
                        { 
                            $("#terms_conditions_error").show();
                        }
                        if (!focusSet) { $("#terms_conditions").focus(); }
                        allfields = false;
                    } else {
                        $("#terms_conditions_error").hide();
                    }  
                    ...
                    if (duration == "" || duration== null) {
                        if ($("#duration").next(".validation").length == 0) // only add if not added
                        {
                            $("#duration").after("<div class='validation' style='color:red;margin-bottom:15px;'>Please Enter Quiz Duration (In Minutes) </div>");
                        }
                        if (!focusSet) { $("#duration").focus(); }
                        allfields = false;
                    } else {
                        $("#duration").next(".validation").remove(); // remove it
                    }     

                    var total_mark = $("#total_mark").val();
                    if (total_mark == "" || total_mark== null) {
                        if ($("#total_mark").next(".validation").length == 0) // only add if not added
                        {
                            $("#total_mark").after("<div class='validation' style='color:red;margin-bottom:15px;'>Please Enter Quiz Total Marks </div>");
                        }
                        if (!focusSet) { $("#total_mark").focus(); }
                        allfields = false;
                    } else {
                        $("#total_mark").next(".validation").remove(); // remove it
                    }

                    var total_question = $("#total_question").val();
                    if (total_question == "" || total_question== null) {
                        if ($("#total_question").next(".validation").length == 0) // only add if not added
                        {
                        ...
                        if (!focusSet) { $("#total_question").focus(); }
                        allfields = false;
                    } else {
                        $("#total_question").next(".validation").remove(); // remove it
                    }

                    var start_date = $("#start_date").val();
                    if (start_date == "" || start_date== null) {
                        if ($("#start_date").next(".validation").length == 0) // only add if not added
                        {
                        ...
                        $("#start_date").next(".validation").remove(); // remove it
                    }

                    var start_time = $("#start_time").val();
                    if (start_time == "" || start_time== null) {
                        if ($("#start_time").next(".validation").length == 0) // only add if not added
                        {
                            $("#start_time").after("<div class='validation' style='color:red;margin-bottom:15px;'>Please Select Quiz Start Time</div>");
                        }
                        if (!focusSet) { $("#start_time").focus(); }
                        allfields = false;
                    } else {
                        $("#start_time").next(".validation").remove(); // remove it
                    }

                    var end_date = $("#end_date").val();
                    if (end_date == "" || end_date== null) {
                        if ($("#end_date").next(".validation").length == 0) // only add if not added
                        {
                    ...
                    if (tprize_img == "" || tprize_img== null) {
                        ...
                        $("#tprize_img").next(".validation").remove(); // remove it
                    }
                    ...
                    if (cprize == "" || cprize== null) {
                        if ($("#cprize").next(".validation").length == 0) // only add if not added
                        {
                            $("#cprize").after("<div class='validation' style='color:red;margin-bottom:15px;'>Please Enter Consolation Prizes </div>");
                        }
                        if (!focusSet) { $("#cprize").focus(); }
                        allfields = false;
                    } else {
                        $("#cprize").next(".validation").remove(); // remove it
                    }

                    var cdetails = $("#cdetails").val();
                    if (cdetails == "" || cdetails== null) {
                        if ($("#cdetails").next(".validation").length == 0) // only add if not added
                        {
                        ...
                        $("#cdetails").next(".validation").remove(); // remove it
                    }
                    var availability_id = $("#availability_id").val();
                    if (availability_id == "" || availability_id== null) {
                        if ($("#availability_id").next(".validation").length == 0) // only add if not added
                        {
                        ...
                        $("#availability_id").next(".validation").remove(); // remove it
                    }

                    var que_bank_id = $("#que_bank_id").val();
                    if (que_bank_id == "" || que_bank_id== null) {
                        if ($("#que_bank_id").next(".validation").length == 0) // only add if not added
                        {
                        ...
                        $("#que_bank_id").next(".validation").remove(); // remove it
                    }
